...
def create_par(ctx_path, ctx: dict,
    ...
    def _resolve_companion(ctx_key: str, ext: str) -> str:
        """Find this AOI's <ext> file. Prefer the path ctx recorded; else
        search this AOI's folder for "<AOI>.<ext>" (and versioned forms like
        "<AOI> (1).<ext>"), taking the most recent; else fall back to the
        canonical "<AOI>.<ext>" name."""
        p = ctx.get(ctx_key)
        if p and (lisflood_dir / Path(p).name).exists():
            return Path(p).name
        matches = sorted(
            lisflood_dir.glob(f"{aoi_name}*.{ext}"),
            key=lambda f: f.stat().st_mtime,
        )
        if matches:
            return matches[-1].name
        return f"{aoi_name}.{ext}"
    ...
    bci_file_name = _resolve_companion("bci_path", "bci")
    bdy_file_name = _resolve_companion("bdy_path", "bdy")

    # This AOI uses a Manning raster (varying friction) when the Manning
    # step produced one — i.e. ``manning_ascii_path`` is set and the file
    # is present.  Otherwise we fall back to a uniform ``fpfric`` value.
    have_manning = bool(ctx.get("manning_ascii_path")) and \
        (lisflood_dir / manning_file_name).exists()
    ...
    if ctx.get("manning_ascii_path") and not (lisflood_dir / manning_file_name).exists():
    ...
    # folder itself, relative to where the .par is run (the lisflood-files
    # folder), so we neither write the full address nor pre-create it here.
    ...
    lines.append("# LISFLOOD-FP parameter file generated by FIMsim.")
    lines.append("")
    ...
    elif use_fpfric and fpfric_val is not None:
    ...
    lines.append(f"dirroot         {results_dir_name}")
    ...
    ctx["par_dirroot"]         = results_dir_name
