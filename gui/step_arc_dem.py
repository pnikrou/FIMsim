"""ARC-Curve2Flood Step 3 — DEM.

Two layouts based on AOI count:
  * 1 AOI   → one DEMConfigPanel embedded directly.
  * >1 AOIs → accordion of AOIDEMCard widgets, one per AOI.

Output: GeoTIFF in <AOI>/dem/ (NenCarta's dem_dir).
No ASCII conversion needed — ARC uses raw GeoTIFFs.
"""
import re
...
from core.arc_orchestrate import run_arc_dem_all
...
class StepArcDEMWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Stack: single-AOI panel (index 0) vs multi-AOI accordion (index 1)
        ...
        # Two-column: info table (left 30%) + raster canvas (right 70%)
    ...
    def _build_results(self, ctx):
        ...
        for entry in per_aoi:
            ...
            rl.addWidget(btn, 1)
            # Show the dem/ folder path as a hint
            dem_dir = str(Path(path).parent) if path else "—"
            hint = QLabel(f"<small>{dem_dir}</small>")
    ...
    def _populate_dem_info(self, name: str, path: str):
        ...
        try:
            ...
            with rasterio.open(path) as src:
                ...
                rows.append(("Folder (dem_dir)", str(Path(path).parent)))
    ...
    def _on_message(self, msg):
        ...
        if m:
            ...
            self._status_lbl.setText(
                ...
                + (f"  Starting DEM {i + 1} / {total} …" if i < total else "")
            )
            return
    ...
    def _run_step(self):
        ...
        f0 = self._features[0]
        check_files = [str(Path(f0.folder_path) / "dem" / "dem.tif")]
        ...
        for cfg, feat in zip(per_aoi, self._features):
            if cfg.get("has_dem"):
                ...
                if not paths:
                    self._error_lbl.setText(
                        f"AOI '{feat.name}' is set to 'I have a DEM raster' "
