"""Per-AOI DEM source configuration card.

Mirrors AOIManningCard / AOIBCICard: collapsible accordion row that hosts
a DEMConfigPanel (Download from 3DEP vs I have a DEM raster).
"""
...
class AOIDEMCard(QFrame):
    ...
    def _refresh_status(self):
        ...
        cell_str = f"{cfg.get('dem_res_m', 10):g} m"
        if cfg["has_dem"]:
            ...
            src = "<i>Source:</i> 3DEP download"
        self._status_lbl.setText(
            f"{src} &nbsp;·&nbsp; <i>Cell:</i> {cell_str}"
        )
