...
class StepMultiAOIWidget(QWidget):
    ...
    def __init__(self, log_fn, model: str = "lisflood", parent=None):
        ...
        # LISFLOOD-FP runs on a rectangular raster domain, so an irregular
        # uploaded AOI is replaced by its bounding-box rectangle for every
        # downstream step (the original is kept and shown in the preview).
    ...
    def _persist_features(self, features):
        ...
        for f in features:
            aoi_list.append({
                ...
                # rectangle every downstream step uses; these record what the
