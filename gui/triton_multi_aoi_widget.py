...
class MultiAOIWidget(QWidget):
    ...
    def __init__(self, log_fn, parent=None, rectangularize: bool = False):
        """``rectangularize``: when True (TRITON), any confirmed AOI that is
    ...
    def _confirm(self):
        ...
        # TRITON: the model domain must be a rectangle.  Replace any irregular
        # polygon with its bounding box for every downstream step, keeping the
        # original so the preview can show both.
        if self._rectangularize:
            from core.aoi_bbox import rectangularize_features
            self._log("Checking AOI shape(s) — TRITON runs on a rectangular "
                      "domain …")
    ...
    def _render_detail_panel(self, feature: AOIFeatureInfo):
        ...
        # Rectangular-domain note (TRITON).  Say plainly which file the
